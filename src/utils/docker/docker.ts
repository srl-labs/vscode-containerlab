...
export async function checkAndPullDockerImage(image: string, imagePullPolicy: ImagePullPolicy): Promise<boolean> {
  if (!dockerClient) {
    outputChannel.debug("pullDockerImage() failed: docker client unavailable.")
    return false;
  }
  ...
  if(!imageExists) {
    switch (imagePullPolicy) {
      case ImagePullPolicy.Never:
        outputChannel.debug(`Pull policy is 'never', skipping image pull for missing image ${image}`);
        break;
