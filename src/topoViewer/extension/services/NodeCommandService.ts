...
/**
 * Creates a default interface object for command execution
 */
 ...
 * Executes VS Code commands for SSH, shell attach, logs, and packet capture.
 */
export class NodeCommandService {
   ...
   */
  async handleNodeEndpoint(endpointName: string, payloadObj: any): Promise<EndpointResult> {
   ...
   */
  async handleInterfaceEndpoint(endpointName: string, payloadObj: any): Promise<EndpointResult> {
    let result: unknown = null;
    let error: string | null = null;
    ...
    switch (endpointName) {
      case 'clab-interface-capture': {
        try {
          ...
          const actualInterfaceName = await this.resolveInterfaceName(data.nodeName, data.interfaceName);
          const iface = createInterfaceObject(
            data.nodeName,
            actualInterfaceName,
            data.interfaceName !== actualInterfaceName ? data.interfaceName : ''
          );
