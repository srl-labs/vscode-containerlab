...
import {
  ...
  extractNodeIcons,
  getIconDataUriForRole
} from "./managerCytoscapeBaseStyles";
...
const ID_NODE_CERT_KEYSIZE_FILTER_INPUT =
  "node-cert-key-size-dropdown-container-filter-input" as const;
...
export class ManagerNodeEditor {
  ...
  private updateMdaCardSlotLabels(componentIdx: number): void {
    ...
    if (xiomContainer) {
      ...
      xiomRows.forEach((row) => {
        ...
        const xiomSlotRaw = this.getInputValue(
  ...
  private isCpmSlot(v: string): boolean {
  ...
  private initMdaTypeDropdown(componentIdx: number, mdaId: number): void {
    ...
    createFilterableDropdown(
      ...
      initial,
      (selected: string) =>
        this.setInputValue(`component-${componentIdx}-mda-${mdaId}-type`, selected),
      PH_SEARCH_TYPE,
      true
    );
  }
  ...
  private initXiomTypeDropdown(componentIdx: number, xiomId: number): void {
    ...
    createFilterableDropdown(
      ...
      initial,
      (selected: string) =>
        this.setInputValue(`component-${componentIdx}-xiom-${xiomId}-type`, selected),
      PH_SEARCH_TYPE,
      true
    );
  }
  ...
  private initXiomMdaTypeDropdown(componentIdx: number, xiomId: number, mdaId: number): void {
    const initial = this.getInputValue(
    ...
    createFilterableDropdown(
      ...
      initial,
      (selected: string) =>
        this.setInputValue(`component-${componentIdx}-xiom-${xiomId}-mda-${mdaId}-type`, selected),
      PH_SEARCH_TYPE,
      true
    );
  }
      ...
      if (xiomContainer) {
        ...
        xiomRows.forEach((row) => {
          ...
          if (xiomId !== null) {
            ...
            const xmdaRows = Array.from(
              row.querySelectorAll(`.${CLASS_COMPONENT_XIOM_MDA_ENTRY}`)
            ) as HTMLElement[];
  ...
  private applyIconEditorSelection(): void {
    ...
    this.setIconCornerRadius(
      Number.isFinite(cornerRadius) ? cornerRadius : DEFAULT_ICON_CORNER_RADIUS
    );
  ...
  private registerComponentEntryHandlers(): void {
    (window as any).addComponentEntry = () => {
      this.addComponentEntry(undefined, { slotType: "card" });
    };
    (window as any).addCpmComponentEntry = () => {
      this.addComponentEntry(undefined, { slotType: "cpm" });
    };
    (window as any).addCardComponentEntry = () => {
      this.addComponentEntry(undefined, { slotType: "card" });
    };
    (window as any).addIntegratedMdaEntry = () => {
      this.addIntegratedMdaEntry();
    };
  }
   ...
   */
  private addDynamicKeyValueEntry(
  ...
  private refreshIconDropdownAfterIconChange(preferredIcon?: string): void {
    ...
    if (shapeSelect) {
      this.populateIconShapeOptions(shapeSelect);
      const targetIcon =
        preferredIcon && availableIcons.includes(preferredIcon) ? preferredIcon : selectedIcon;
  ...
  private setupCustomNodeFields(node: cytoscape.NodeSingular): void {
    ...
    if (heading) {
      // Update only the .panel-title span to preserve the close button
  ...
  private loadConfigurationTab(extraData: Record<string, any>, actualInherited: string[]): void {
    ...
    this.populateKeyValueProperty(
      extraData,
      CN_ENV,
      CN_ENV,
      ID_NODE_ENV_CONTAINER,
      actualInherited
    );
    this.populateArrayProperty(
      extraData,
  ...
  private populateArrayProperty(
    ...
    if (Array.isArray(values)) {
      values.forEach((value: string) =>
        this.addDynamicEntryWithValue(containerName, value, placeholder)
      );
    }
  ...
  private loadNetworkTab(extraData: Record<string, any>, actualInherited: string[]): void {
    ...
    if (extraData.ports && Array.isArray(extraData.ports)) {
      extraData.ports.forEach((port: string) =>
        this.addDynamicEntryWithValue(CN_PORTS, port, PH_PORT)
      );
    }
    ...
    if (extraData.dns && extraData.dns.servers && Array.isArray(extraData.dns.servers)) {
      extraData.dns.servers.forEach((server: string) =>
        this.addDynamicEntryWithValue(CN_DNS_SERVERS, server, PH_DNS_SERVER)
      );
    }
    ...
    if (extraData.aliases && Array.isArray(extraData.aliases)) {
      extraData.aliases.forEach((alias: string) =>
        this.addDynamicEntryWithValue(CN_ALIASES, alias, PH_ALIAS)
      );
    }
  ...
  private loadCapAdd(extraData: Record<string, any>, actualInherited: string[]): void {
    if (extraData[PROP_CAP_ADD] && Array.isArray(extraData[PROP_CAP_ADD])) {
      extraData[PROP_CAP_ADD].forEach((cap: string) =>
        this.addDynamicEntryWithValue(CN_CAP_ADD, cap, PH_CAP)
      );
    }
  ...
  private loadDevices(extraData: Record<string, any>, actualInherited: string[]): void {
    if (extraData.devices && Array.isArray(extraData.devices)) {
      extraData.devices.forEach((device: string) =>
        this.addDynamicEntryWithValue(CN_DEVICES, device, PH_DEVICE)
      );
    }
    ...
    if (extraData.certificate) {
      ...
      if (extraData.certificate.sans && Array.isArray(extraData.certificate.sans)) {
        extraData.certificate.sans.forEach((san: string) =>
          this.addDynamicEntryWithValue(CN_SANS, san, PH_SAN)
        );
      }
    }
  }
  ...
  private loadImagePullPolicy(extraData: Record<string, any>, actualInherited: string[]): void {
    ...
    this.markFieldInheritance(
      ID_NODE_IPP_DROPDOWN,
      actualInherited.includes(PROP_IMAGE_PULL_POLICY)
    );
  }
   ...
   */
  private addDynamicEntryWithValue(
    containerName: string,
    value: string,
    placeholder: string
  ): void {
   ...
   */
  private addDynamicKeyValueEntryWithValue(
    containerName: string,
    key: string,
    value: string
  ): void {
  ...
  private collectDynamicKeyValueEntries(containerName: string): Record<string, string> {
    ...
    entries.forEach((entry: Element) => {
      const keyInput = entry.querySelector(
        `input[data-field="${containerName}-key"]`
      ) as HTMLInputElement;
      const valueInput = entry.querySelector(
        `input[data-field="${containerName}-value"]`
      ) as HTMLInputElement;
  ...
  private async saveCustomNodeTemplate(
    name: string,
    nodeProps: NodeProperties,
    setDefault: boolean,
    oldName?: string
  ...
  private commitXiomMdaDropdowns(idx: number, xiomId: number, xiomRow: HTMLElement): void {
    const xmdaRows = Array.from(
      xiomRow.querySelectorAll(`.${CLASS_COMPONENT_XIOM_MDA_ENTRY}`)
    ) as HTMLElement[];
    for (const xmda of xmdaRows) {
      const mdaId = this.extractIndex(xmda.id, /component-\d+-xiom-\d+-mda-entry-(\d+)/);
      if (mdaId === null) continue;
      const mdaFilter = document.getElementById(
        `component-${idx}-xiom-${xiomId}-mda-${mdaId}-type-dropdown-filter-input`
      ) as HTMLInputElement | null;
      const mdaHidden = document.getElementById(
  ...
  private collectXiomMdas(componentIdx: number, xiomId: number): any[] {
    ...
    for (const row of rows) {
      ...
      if (mdaId === null) continue;
      const slotRaw = this.getInputValue(
        `component-${componentIdx}-xiom-${xiomId}-mda-${mdaId}-slot`
      ).trim();
      const typeVal = this.getInputValue(
        `component-${componentIdx}-xiom-${xiomId}-mda-${mdaId}-type`
      ).trim();
      const mda: any = {};
      if (/^\d+$/.test(slotRaw)) mda.slot = parseInt(slotRaw, 10);
  ...
  private collectRuntimeProps(nodeProps: NodeProperties): void {
    ...
    const rpVal =
      (document.getElementById(ID_NODE_RP_FILTER_INPUT) as HTMLInputElement | null)?.value || "";
  ...
  private collectNetworkProps(nodeProps: NodeProperties): void {
    ...
    const nmVal =
      (document.getElementById(ID_NODE_NM_FILTER_INPUT) as HTMLInputElement | null)?.value || "";
  ...
  private collectCertificateProps(nodeProps: NodeProperties): void {
    ...
    const keySize =
      (document.getElementById(ID_NODE_CERT_KEYSIZE_FILTER_INPUT) as HTMLInputElement | null)
        ?.value || "";
  ...
  private collectHealthcheckProps(nodeProps: NodeProperties): void {
    ...
    const ippVal =
      (document.getElementById(ID_NODE_IPP_FILTER_INPUT) as HTMLInputElement | null)?.value || "";
    if (ippVal && ippVal !== LABEL_DEFAULT) nodeProps["image-pull-policy"] = ippVal as any;

    const runtimeVal =
      (document.getElementById(ID_NODE_RUNTIME_FILTER_INPUT) as HTMLInputElement | null)?.value ||
      "";
