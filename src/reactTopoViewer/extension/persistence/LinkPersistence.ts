/**
 * LinkPersistence - Handles link CRUD operations in YAML documents
 */
 ...
 */
export function addLink(doc: YAML.Document.Parsed, linkData: LinkSaveData): SaveResult {
  try {
 ...
 */
export function deleteLink(doc: YAML.Document.Parsed, linkData: LinkSaveData): SaveResult {
  try {
    const linksSeq = doc.getIn(['topology', 'links'], true) as YAML.YAMLSeq | undefined;
    if (!linksSeq || !YAML.isSeq(linksSeq)) {
      return { success: false, error: ERROR_LINKS_NOT_SEQ };
