...
export function resolveContainerAndInterface(params: {
  ...
  const {
    parsed, nodeName, actualNodeName, ifaceName, fullPrefix,
    clabName, includeContainerData, clabTreeData,
  } = params;
...
export function extractEdgeInterfaceStats(ifaceData: unknown): Record<string, number> | undefined {
  ...
  const keys = [
    'rxBps', 'rxPps', 'rxBytes', 'rxPackets',
    'txBps', 'txPps', 'txBytes', 'txPackets', 'statsIntervalSeconds',
  ];
...
export function createClabInfo(params: {
  ...
  sourceIface: string;
  targetIface: string;
  sourceIfaceData?: Record<string, unknown>;
  targetIfaceData?: Record<string, unknown>;
}): Record<string, unknown> {
  const {
    sourceContainerName, targetContainerName, sourceIface, targetIface,
    sourceIfaceData = {}, targetIfaceData = {},
  } = params;

  const {
    mac: srcMac = '', state: srcState = '', mtu: srcMtu = '', type: srcType = '',
  } = sourceIfaceData;

  const {
    mac: tgtMac = '', state: tgtState = '', mtu: tgtMtu = '', type: tgtType = '',
  } = targetIfaceData;
...
export function extractExtLinkProps(linkObj: Record<string, unknown>): Record<string, unknown> {
  const {
    type: extType = '', mtu: extMtu = '', vars: extVars, labels: extLabels,
    'host-interface': extHostInterface = '', mode: extMode = '', remote: extRemote = '',
    vni: extVni = '', 'dst-port': extDstPort = '', 'src-port': extSrcPort = '',
  } = linkObj ?? {};
...
export function buildEdgeExtraData(params: {
  ...
  sourceIface: string;
  targetIface: string;
  sourceIfaceData: Record<string, unknown> | undefined;
  targetIfaceData: Record<string, unknown> | undefined;
  extValidationErrors: string[];
  sourceNodeId: string;
  targetNodeId: string;
}): Record<string, unknown> {
  const {
    linkObj, endA, endB, sourceContainerName, targetContainerName,
    sourceIface, targetIface, sourceIfaceData, targetIfaceData,
    extValidationErrors, sourceNodeId, targetNodeId,
  } = params;
...
export function buildEdgeElement(params: {
  ...
  const {
    linkObj, endA, endB, sourceNode, targetNode, sourceIface, targetIface,
    actualSourceNode, actualTargetNode, sourceContainerName, targetContainerName,
    sourceIfaceData, targetIfaceData, edgeId, edgeClass, specialNodes,
  } = params;
  ...
  const extraData = buildEdgeExtraData({
    linkObj, endA, endB, sourceContainerName, targetContainerName,
    sourceIface, targetIface, sourceIfaceData, targetIfaceData,
    extValidationErrors, sourceNodeId: sourceNode, targetNodeId: targetNode,
  });
...
export function addEdgeElements(
  ...
  for (const linkObj of topology.links) {
    ...
    const { node: sourceNode, iface: sourceIface } = splitEndpoint(endA as string | { node: string; interface?: string });
    const { node: targetNode, iface: targetIface } = splitEndpoint(endB as string | { node: string; interface?: string });
    ...
    const sourceInfo = resolveContainerAndInterface({
      parsed, nodeName: sourceNode, actualNodeName: actualSourceNode, ifaceName: sourceIface,
      fullPrefix, clabName, includeContainerData: opts.includeContainerData, clabTreeData: opts.clabTreeData,
    });
    const targetInfo = resolveContainerAndInterface({
      parsed, nodeName: targetNode, actualNodeName: actualTargetNode, ifaceName: targetIface,
      fullPrefix, clabName, includeContainerData: opts.includeContainerData, clabTreeData: opts.clabTreeData,
    });
    ...
    const edgeEl = buildEdgeElement({
      linkObj: linkObj as Record<string, unknown>, endA, endB, sourceNode, targetNode,
      sourceIface, targetIface, actualSourceNode, actualTargetNode,
      sourceContainerName, targetContainerName,
      sourceIfaceData: sourceIfaceData as Record<string, unknown> | undefined,
      targetIfaceData: targetIfaceData as Record<string, unknown> | undefined,
      edgeId, edgeClass, specialNodes,
    });
