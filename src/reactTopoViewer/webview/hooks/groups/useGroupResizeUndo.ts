...
export function useGroupResizeUndo(options: UseGroupResizeUndoOptions): UseGroupResizeUndoReturn {
  ...
  const onResizeStart = useCallback((groupId: string) => {
  ...
  const onResizeEnd = useCallback((
    ...
    log.info(`[GroupResizeUndo] Resize completed for group ${groupId}: ${startState.groupBefore.width}x${startState.groupBefore.height} -> ${finalWidth}x${finalHeight}`);
