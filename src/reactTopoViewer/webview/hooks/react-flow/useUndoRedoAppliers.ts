...
function applyPropertyEditAction(action: UndoRedoActionPropertyEdit, isUndo: boolean): void {
  ...
  log.info(
    `[UndoRedo] Applied ${action.entityType} property ${isUndo ? "undo" : "redo"} for ${action.entityId}`
  );
}
  ...
  return (action: UndoRedoAction, isUndo: boolean) => {
    switch (action.type) {
      ...
      case ACTION_GRAPH:
        ...
        for (const change of isUndo ? action.before : action.after)
          applySingleChange(change, isUndo, ctx);
 ...
 */
export function useUndoRedoAppliers(
  options: UseUndoRedoAppliersOptions
): UseUndoRedoAppliersReturn {
  const {
    setNodePositions,
    addNode,
    addEdge,
