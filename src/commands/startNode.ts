...
export async function startNode(node: ContainerlabNode) {
  ...
  // e.g. ["sudo", "docker", "start", "<containerId>"]
  const cmdArgs = useSudo
    ? ["sudo", "docker", "start", containerId]
    : ["docker", "start", containerId];

  // Print the exact command
  outputChannel.appendLine(`[startNode] Running: ${cmdArgs.join(" ")}`);
  ...
  try {
      ...
      async (progress, token) => {
        return new Promise<void>((resolve, reject) => {
          ...
          token.onCancellationRequested(() => {
            child.kill();
            reject(new Error("User canceled the startNode command."));
          });
    ...
    vscode.window
      .showInformationMessage(`Node started: ${containerId}`, "Show Logs")
      .then((choice) => {
        if (choice === "Show Logs") {
    ...
    vscode.window.showErrorMessage(`Failed to start node (${containerId}): ${err.message}`);
  }
}