...
test.describe('Lab Settings Panel', () => {
  ...
  test('lab settings panel has Management tab', async ({ page }) => {
    ...
    const mgmtTab = page.locator(`${SEL_LAB_SETTINGS_PANEL} .panel-tab-button:has-text("Management")`);
  ...
  test('lab settings fields are read-only when canvas is locked', async ({ page, topoViewerPage }) => {
    // Lock the canvas
  ...
  test('can change prefix type to custom and enter custom prefix', async ({ page }) => {
    ...
    const customPrefixInput = page.locator(`${SEL_LAB_SETTINGS_PANEL} input[placeholder="Enter custom prefix"]`);
  ...
  test('Management tab shows network name field', async ({ page }) => {
    ...
    const networkNameLabel = page.locator(`${SEL_LAB_SETTINGS_PANEL} label:has-text("Network Name")`);
