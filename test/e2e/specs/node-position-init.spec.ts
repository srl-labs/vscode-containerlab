...
async function pollNodePosition(page: Page, nodeId: string, iterations: number, intervalMs: number): Promise<Array<{ time: number; pos: { x: number; y: number } }>> {
...
function detectDrift(
  ...
  for (const nodeId of Object.keys(annotationPositions)) {
    ...
    if (deltaT0T2 > 5) {
      console.log(`[DELAYED DRIFT] ${nodeId}: T0=(${atT0.x}, ${atT0.y}) -> T2=(${atT2.x}, ${atT2.y})`);
      driftDetected = true;
    }
    ...
    if (deltaFromFile > tolerance) {
      console.log(`[DRIFT FROM FILE] ${nodeId}: expected=(${expected.x}, ${expected.y}), got=(${atT2.x}, ${atT2.y}), react=(${inReact?.x}, ${inReact?.y})`);
      driftDetected = true;
    }
    ...
    if (inReact) {
      ...
      if (deltaReactCy > 5) {
        console.log(`[REACT VS CY] ${nodeId}: React=(${inReact.x}, ${inReact.y}), Cy=(${atT2.x}, ${atT2.y})`);
      }
    }
  }
...
test.describe('Node Position Initialization', () => {
  test('nodes load at positions from annotations file without drift', async ({ topoViewerPage }) => {
    ...
    expect(spine1DeltaX, `spine1 X drift: expected ${spine1Expected!.x}, got ${spine1Actual.x}`).toBeLessThan(tolerance);
    expect(spine1DeltaY, `spine1 Y drift: expected ${spine1Expected!.y}, got ${spine1Actual.y}`).toBeLessThan(tolerance);
    ...
    expect(border1DeltaX, `border1 X drift: expected ${border1Expected!.x}, got ${border1Actual.x}`).toBeLessThan(tolerance);
    expect(border1DeltaY, `border1 Y drift: expected ${border1Expected!.y}, got ${border1Actual.y}`).toBeLessThan(tolerance);
    ...
    expect(leaf1DeltaX, `leaf1 X drift: expected ${leaf1Expected!.x}, got ${leaf1Actual.x}`).toBeLessThan(tolerance);
    expect(leaf1DeltaY, `leaf1 Y drift: expected ${leaf1Expected!.y}, got ${leaf1Actual.y}`).toBeLessThan(tolerance);
  });
  ...
  test('spine-leaf topology preserves node positions on load', async ({ topoViewerPage }) => {
    ...
    if (spine1Expected) {
      ...
      expect(deltaX, `spine1 X drift: expected ${spine1Expected.x}, got ${spine1Actual.x}`).toBeLessThan(tolerance);
      expect(deltaY, `spine1 Y drift: expected ${spine1Expected.y}, got ${spine1Actual.y}`).toBeLessThan(tolerance);
    }
    ...
    if (leaf1Expected) {
      ...
      expect(deltaX, `leaf1 X drift: expected ${leaf1Expected.x}, got ${leaf1Actual.x}`).toBeLessThan(tolerance);
      expect(deltaY, `leaf1 Y drift: expected ${leaf1Expected.y}, got ${leaf1Actual.y}`).toBeLessThan(tolerance);
    }
  });
  ...
  test('rapid file switch does not cause position drift', async ({ topoViewerPage }) => {
    ...
    expect(deltaX, `spine1 X drift after file switch: expected ${spine1Expected!.x}, got ${spine1Actual.x}`).toBeLessThan(tolerance);
    expect(deltaY, `spine1 Y drift after file switch: expected ${spine1Expected!.y}, got ${spine1Actual.y}`).toBeLessThan(tolerance);
  });
  ...
  test('position drift detection with logging', async ({ page, topoViewerPage }) => {
    ...
    for (const ann of annotations.nodeAnnotations || []) {
      ...
      if (deltaX > tolerance || deltaY > tolerance) {
        console.log(`[DRIFT] ${ann.id}: expected (${ann.position.x}, ${ann.position.y}), got (${actual.x}, ${actual.y}), delta: (${deltaX}, ${deltaY})`);
        driftDetected = true;
      }
    }
  ...
  test('delayed position drift - React state vs annotations vs Cytoscape', async ({ page, topoViewerPage }) => {
    ...
    console.log('[T0] Positions immediately after canvas ready:', JSON.stringify(positionsT0, null, 2));
    ...
    const driftDetected = detectDrift(annotationPositions, positionsT0, positionsT2, reactPositions, POSITION_TOLERANCE);
