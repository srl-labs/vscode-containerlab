...
function validateNetworkNodeAnnotation(
  annotations: {
    networkNodeAnnotations?: Array<{ id: string; type: string; position?: { x: number; y: number } }>;
  },
  networkId: string,
  networkType: string
): void {
...
test.describe('Network Node Position Persistence', () => {
  ...
  test('host network position persists after reload', async ({ page, topoViewerPage }) => {
    ...
    const netAnn = annotationsBefore.networkNodeAnnotations?.find((n: { id: string }) => n.id === hostId);
...
test.describe('Network Node Undo/Redo', () => {
  ...
  test('mgmt-net network deletion and undo restores link', async ({ page, topoViewerPage }) => {
    ...
    console.log(BROWSER_LOGS_LABEL);
    consoleLogs
      .filter(log => log.includes('Services') || log.includes('link') || log.includes('Link') || log.includes('error') || log.includes('Error'))
      .forEach(log => console.log(log));
  });
  ...
  test('mgmt-net link undo/redo with existing topology', async ({ page, topoViewerPage }) => {
    ...
    const netAnn = annotations.networkNodeAnnotations?.find((n: { id: string }) => n.id === mgmtNetId);
    expect(netAnn).toBeDefined();
    expect(netAnn?.type).toBe('mgmt-net');
  });

  test('comprehensive undo/redo workflow for network nodes and links', async ({ page, topoViewerPage }) => {
    // ============================================================================
    // SETUP
    // ============================================================================
