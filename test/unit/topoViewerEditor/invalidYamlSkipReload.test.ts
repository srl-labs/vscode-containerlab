...
describe('TopoViewerEditor invalid YAML prevention', () => {
  after(() => {
  ...
  beforeEach(() => {
    ...
    vscodeStub.workspace.onDidSaveTextDocument = () => ({
      dispose: () => {},
    });
  });

  it('should not reload webview when validation fails', async () => {
    sinon.stub(fs.promises, 'readFile').resolves(sampleYaml as any);
    sinon.stub(fs.promises, 'writeFile').resolves();
    sinon.stub(fs.promises, 'mkdir').resolves();

    let saveCallback: any = null;
    vscodeStub.workspace.onDidSaveTextDocument = (cb: any) => {
      saveCallback = cb;
      return { dispose: () => {} };
    };
    ...
    const validateStub = sinon.stub(editor as any, 'validateYaml');
    validateStub.onFirstCall().resolves(true);
    validateStub.onSecondCall().resolves(false);

    await editor.createWebviewPanel(context, vscodeStub.Uri.file('/tmp/test.clab.yml'), 'test');

    if (saveCallback) {
      saveCallback({ uri: vscodeStub.Uri.file('/tmp/test.clab.yml') });
      await new Promise(resolve => setTimeout(resolve, 0));
    }

    expect((panelStub.webview.postMessage as sinon.SinonSpy).calledWithMatch({ type: 'yaml-saved' })).to.be.false;
  });
});
