...
describe('TopoViewerEditor manual save reload', () => {
  after(() => {
  ...
  it('should reload webview when yaml is manually saved', async () => {
  ...
  sinon.stub(vscodeStub.window, 'createWebviewPanel').returns(panelStub as any);
    ...
    expect((panelStub.webview.postMessage as sinon.SinonSpy).calledWithMatch({ type: 'yaml-saved' })).to.be.true;
  });
});
